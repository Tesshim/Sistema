...
if(isset($_POST['done'])){
    ...
    $sql = mysql_query("SELECT cpf FROM usuarios WHERE email = '$email'") or die(mysql_error());
	
            if(mysql_num_rows($sql)){
				$sqlarray = mysql_fetch_assoc($sql);
				$cpf = $sqlarray["cpf"];
				$consultapass = mysql_query("SELECT pass FROM login WHERE cpf = '$cpf'") or die(mysql_error());
				$pass = mysql_fetch_assoc($consultapass);
                $erro = "Sua senha foi enviada para o e-mail especificado!";
				
				send_mail(	'naoresponda@proace.ufvjm.edu.br',
						$email,
						'Recuperação de Senha - DASA/PROACE',
						'Sua senha é: '.md5($pass["pass"]).'');
    ...
    <div id="main">
	  ...
	  <center>
        ...
        <div class="container">
			...
			<?php
	echo '<form action="validardados.php" method="post">';
	if(isset($erro)){
...
//<td><input name="email" type="text" class="campo" id="email" /></td><td><input type="submit" value="Enviar Senha" /><input type="hidden" name="done" value="" />
//</tr></table></tbody></form>';
echo 'Serviço não disponível no momento.';
